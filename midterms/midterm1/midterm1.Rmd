...
wolves %>% 
  ...
  group_by(park) %>% 
  summarize(pack_total=sum(packsize_aug, na.rm=T)) %>% 
  arrange(desc(pack_total))
```
...
wolves %>% 
  ...
  group_by(park) %>% 
  summarize(mort_all_total=sum(mort_all, na.rm=T)) %>% 
...
```
#### The Mollie wolf pack hunts bison, which provide a lot of food for them and other scavengers. They regularly interact with bears because of their bison hunting which no other wolf pack does. 
