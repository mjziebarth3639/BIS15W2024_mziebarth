...
removes scientific notation, always run with ggplot
```{r}
...
```{r}
p + geom_point(aes(color=thermoregulation), size=1.5) #for scatter plots, color is the same as fill in barplots
```
...
homerange %>% 
  ...
  geom_bar() +
  coord_flip()+
  labs(title="Obersevations by Taxon", 
       x="Taxonomic Group")
```
...
homerange %>% 
  ...
  geom_boxplot(alpha=0.4) #transparcy
```
