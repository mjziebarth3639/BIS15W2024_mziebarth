...
ui <- dashboardPage(
  ...
  dashboardBody(
   fluidPage(titlePanel("Ethnicities Admitted at UC Campuses and Over the Years")),
    fluidRow(
      box(width=3,
      ...
      box(width=3,
      selectInput("y", "Select Year", choices=unique(UC_admit$academic_yr)), hr()
),
      box(width=9, 
...
ui <- dashboardPage(
  ...
  dashboardBody(
    fluidPage(titlePanel("Enrollment Across teh UC Campuses Over the Years and by Ethnicity")),
    fluidRow(
      box(width=3,
...
server <- function(input, output, session)  {
  ...
  UC_admit %>% 
    ...
    ggplot(aes(x=campus, y=filtered_count_fr, fill=campus))+
    geom_col()+
    scale_y_log10() +
    theme(axis.text.x = element_text(angle = 60, hjust=1))
  })
