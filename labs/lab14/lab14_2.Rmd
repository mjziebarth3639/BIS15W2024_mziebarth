...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    homerange %>% 
      filter(taxon==input$x) %>% #reactive part is filter
      ggplot(aes(x=log10.hra))+
...
server <- function(input, output, session) { 
  session$onSessionEnded(stopApp) #can't forget, won't show graph without
  output$plot <- renderPlot({
  wolves %>% 
    filter(sex!="NA") %>% 
      filter(pop==input$x) %>% #filter is reactive part, thats where input$x goes
    ggplot(aes(x=sex, fill=sex))+
    geom_bar()
  })
}
