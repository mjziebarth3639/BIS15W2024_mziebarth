...
1. What is AI?
start typing and tab to complete in copilot
2. What is ChatGPT?
...
```{r}
#use a glimpse function to get a glimpse of the netflix data
glimpse(netflix)
```
...
```{r} 
# use filter to sort out teh movies form the netflix data
```
...
Which director has the most movies on netflix? Remove the NA's
```{r}
filter(count(netflix, director, sort=T, na.rm=T), !is.na(director))
```
...
```{r}
#in R how do I use separate with a comma into 2 new columns Chat GPT
#netflix <- separate_rows(netflix, directors, sp=",")
```
...
Make a bar plot of the top directors with the most movies on netflix remove NA's
```{r}
...
Make a bar plot of the top 10 directors with the most movies on netflix with color, remove and NA's
```{r}
...
```{r}
#make a shiny app that displays the top directors with the most movies on netflix
# chat gpt:I want to have a drop box so I can select the release_year 2008, 2010, or, 2020
```
