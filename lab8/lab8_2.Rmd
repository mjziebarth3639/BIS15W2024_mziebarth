...
#need to know how the NA's are represented in the data
#you don't want to do this by default, don't attach it every time
```
...
life_history <- 
  life_history %>% 
  mutate(max_life=na_if(max_life, 0)) #replaces na in data frame
```
...
life_history %>% 
  replace_with_na(replace = list(newborn = "not measured", 
                                 ...
                                 gestation= -999, 
                                 mass= -999)) %>% 
miss_var_summary() #very specific , replaces by variable/how the variable is represented as na
```
