...
```{r}
life_history %>% 
  count(order, sort=T) %>% #specifies x and y
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=reorder(order, mean_mass),y=mean_mass))+ #allows us to order the x axis by mean_mass 
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
...
elephants %>% 
  ...
  geom_jitter(na.rm=T)+#use jitters for 2 continuous variables
...
elephants %>% 
  ggplot(aes(x=age, y=height)) + 
  geom_point() + 
  geom_smooth(method=lm, se=F)+ #se = standard error , which shows against line when se=T
  labs(title="Elephant Age vs. Height", #adds a title
       x="Age (years)",
       y="Height (cm)")+
  theme(plot.title = element_text(size=rel(1.5), hjust = 0.5)) 
#rel is in relation to other texts, it is 1.5x as big
#hjust moves it from left to right, .5 = middle 1 = right
```
