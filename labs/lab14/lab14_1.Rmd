...
1.  The `ui` function controls the user inputs and the way the app will be displayed; i.e. this controls how your app looks.  #user intetrface, controls looks 
...
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm") #x=varibale name select x is variables on x axis, selected = start on bill length
)
...
ui <- fluidPage(
    ...
    plotOutput("plot", width="500px", height="400px") #providing the size/aspect of the vidual
)
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({ #make a plot, can make a table
    
            ggplot(data=homerange, aes_string(x="locomotion", fill=input$x))+ #reactive=input variable
